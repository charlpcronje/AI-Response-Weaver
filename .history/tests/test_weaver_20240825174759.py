...
def weaver(tmp_path, mock_repo):
    ...
    return AIResponseWeaver(str(file_to_monitor), str(log_folder), debug=True)


def test_extract_file_info(weaver):
    code = "# File: test/example.py\nprint('Hello')"
    file_info = weaver.extract_file_info(code, 'python')
    assert file_info == "test/example.py"
...
def test_end_to_end(weaver, tmp_path, mock_repo):
    ...
    # Check results
    created_file = os.path.join(os.path.dirname(
        weaver.file_to_monitor), "test", "example.py")
    assert os.path.exists(created_file)
    with open(created_file, 'r') as f:
