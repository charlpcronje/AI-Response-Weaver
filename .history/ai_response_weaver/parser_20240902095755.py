...
class CustomParser:
    ...
    def _handle_scanning_state(self, line: str):
        ...
        if line.strip().startswith('```'):
            self.code_block_stack.append(self._determine_code_block_type(line))
            self.current_code_block_type = self.code_block_stack[-1]
            self.current_code_block = []
    ...
    def _handle_scanning_state_bak(self, line: str):
        """
        Handle the scanning state of the parser.
    ...
    def _determine_code_block_type(self, line: str) -> str:
        ...
        Returns:
            str: The type of code block ('file', 'instruction', or 'content').
        """
        extension = self._extract_file_extension(line)
    ...
    def _handle_code_block_start(self, line: str):
        code_block_type = self._determine_code_block_type(line)
        file_path = self._extract_file_path(line)
        ...
        if not file_path:
            ...
            self.ui.display_code_block('\n'.join(
                self.current_code_block), self.current_line_number, len(self.current_code_block))

            action = self.ui.prompt_for_code_block_action(code_block_type)
        ...
        self.current_code_block_type = code_block_type

    def _handle_code_block_end(self):
        """
        Handle the end of a code block.
        """
        if self.current_code_block_type == 'file':
            if self.current_file:
                self.current_file.content = self.current_code_block
                self.files.append(self.current_file)
                self.current_file = None
        elif self.current_code_block_type == 'instruction':
            self.instruction_blocks.extend(
                [(line, self.current_line_number + i) for i, line in enumerate(self.current_code_block)])
            self.instruction_block_count += 1
        else:  # 'content'
            # Process nested code blocks if necessary
            self._process_nested_code_blocks(self.current_code_block)
