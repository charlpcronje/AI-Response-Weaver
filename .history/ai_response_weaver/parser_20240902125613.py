...
class CustomParser:
    ...
    def _handle_code_block_start(self, line: str):
        ...
        This method processes the beginning of a code block, handling nested blocks,
        prompting for user decisions when necessary, and collecting the content of the block.

        Args:
            line (str): The line containing the code block start.
        """
        logging.debug(f"Handling code block start: {line}")
        file_path = self._extract_file_path(line)
        ...
        while True:
            ...
            if next_line is None or next_line.strip().startswith('```'):
                break
            self.current_line_index += 1  # Advance to the next line
            self.current_code_block.append(next_line)

        if self.in_parent_block and self.nested_block_decision is None:
            logging.info("Prompting user for nested block decision")
            self.nested_block_decision = self.ui.prompt_for_nested_block_action()
            logging.debug(
                f"User decision for nested block: {self.nested_block_decision}")
            if self.nested_block_decision == 'separate':
                self.current_code_block = []  # Discard parent block content

        # Always display the code block and prompt for action
        logging.info("Displaying code block and prompting for action")
        self.ui.display_code_block('\n'.join(self.current_code_block),
                                   self.current_line_number,
                                   len(self.current_code_block))

        logging.debug(f"File path extracted: {file_path}")
        if not file_path:
            logging.info("No file path found, prompting user for action")
            action = self.ui.prompt_for_code_block_action(
                self.current_code_block_type or "unknown")
            logging.info(f"User action for code block: {action}")
            if action == 'instruction':
                ...
                self.code_block_stack.append(
                    (self.state, self.current_code_block))
            elif action == 'ignore':
                self.state = ParserState.SCANNING
            else:
                logging.warning(
                    f"Unknown action: {action}. Ignoring code block.")
                self.state = ParserState.SCANNING
        else:
        ...
        self.in_parent_block = True
        self.current_code_block_type = self._determine_code_block_type(line)
        logging.debug(f"Code block start handled. Current state: {self.state}")

    def _handle_code_block_end(self):
        """
        Handle the end of a code block, including nested blocks.

        This method processes the end of a code block, updating the parser state
        and handling nested blocks based on user decisions.
        """
        if self.nested_block_decision == 'separate' or not self.in_parent_block:
            if self.current_file:
                # Exclude opening and closing ```
                self.current_file.content = self.current_code_block[1:-1]
                self.files.append(self.current_file)
                self.current_file = None
