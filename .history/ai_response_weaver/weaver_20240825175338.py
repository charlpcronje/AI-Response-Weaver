...
class AIResponseWeaver:
    def __init__(self, file_to_monitor: str, log_folder: str, debug: bool = False):
        ...
        self.debug = debug  # Add this line
    ...
    def process_file(self):
        ...
        if self.debug:
            print(f"DEBUG: Processing file: {self.file_to_monitor}")
        ...
        for language, code in code_blocks:
            ...
            if self.debug:
                print(
                    f"DEBUG: Extracted file path: {file_path} for language: {language}")
            if file_path:
                self.create_or_update_file(file_path, code)
            else:
                logging.warning(
                    f"Could not extract file path from code block (language: {language}): {code[:50]}...")
    ...
    def extract_code_blocks(self):
        ...
        if self.debug:
            print(f"DEBUG: Extracted blocks: {blocks}")
        return blocks
    ...
    def extract_file_info(self, code, language):
        ...
        if self.debug:
            print(
                f"DEBUG: Extracting file info from code: {code[:50]}... Language: {language}")
        first_line = code.split('\n')[0].strip()
        for file_type, config in self.config['file_types'].items():
            if language.lower() in config['extensions'] or any(language.lower().endswith(ext.lstrip('.')) for ext in config['extensions']):
                if self.debug:
                    print(f"DEBUG: Matched file type: {file_type}")
                    print(f"DEBUG: Using regex: {config['regex']}")
                match = re.search(config['regex'], first_line)
                if match:
                    if self.debug:
                        print(f"DEBUG: Regex match found: {match.group(1)}")
                    return match.group(1)
        if self.debug:
            print("DEBUG: No file info extracted")
        return None
    ...
    def create_or_update_file(self, file_path, code):
        ...
        if self.debug:
            print(f"DEBUG: Creating/Updating file: {full_path}")
