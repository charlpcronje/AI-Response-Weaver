...
class CustomParser:
    ...
    def _handle_scanning_state(self, line: str):
        ...
        if line.strip().startswith('```'):
            ...
            file_path = self._extract_file_path_from_code_block(line)
            if file_path:
    ...
    def _handle_code_block_start(self, line: str):
        ...
        if not file_extension:
            file_extension = self._prompt_for_file_extension()
    ...
    def _extract_file_path_from_code_block(self, comment_style: str) -> Optional[str]:
        """
        Extract the file path from the code block using the given comment style.
